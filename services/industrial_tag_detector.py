...
class IndustrialTagDetector:
    ...
    def __init__(self, model_path='sacaria_yolov5n.pt', roi=(0, 0, 0, 0), log_file=None, match_dist=150,
        ...
        try:
            ...
            # Nota sobre "pathlib._local":
            # Em alguns ambientes Windows, a presenca de pacotes conflitantes ou cache antigo do torch.hub
            # pode causar erros do tipo "No module named 'pathlib._local'; 'pathlib' is not a package".
            # Para mitigar proativamente, detectamos um possivel 'pathlib' de site-packages e fazemos
            # um retry com force_reload=True quando o primeiro load falhar.

            def _log_pathlib_origin():
                try:
                    import pathlib as _pl
                    origin = getattr(_pl, "__file__", None)
                    if origin and str(origin).lower().find("site-packages") != -1:
                        log.warning(
                            "Possivel conflito: modulo 'pathlib' carregado de site-packages (%s). "
                            "Considere desinstalar o pacote 'pathlib' do pip para evitar conflitos com o 'pathlib' da biblioteca padrao.",
                            origin,
                        )
                except Exception:
                    pass
            ...
            # Compatibilidade: checkpoints salvos com backport 'pathlib' podem referenciar
            # 'pathlib._local'. Mapeamos este nome para o modulo 'pathlib' carregado
            # (stdlib ou backport) para permitir o unpickle sem instalar nada.
            try:
                import pathlib as _pl
            ...
            try:
                ...
                # Forca recarregar cache do torch.hub (conforme dica do proprio torch.hub)
                log.warning("Falha no primeiro carregamento do YOLOv5 (%s). Tentando force_reload=True...", e_first)
                # Garante o alias tambem no retry (em alguns ambientes o reload recria sys.modules)
                try:
                    import pathlib as _pl
                    sys.modules.setdefault('pathlib._local', _pl)
                except Exception:
        ...
        for obj_id in list(self.tracked_objects.keys()):
            ...
            if flow_mode == "sem_fluxo":
                ...
                if obj['direction'] == -1 and crossed(prev_cy, curr_cy, sub_secondary_line, sub_secondary_dir):
                    # Somente decrementa para "desfazer" um +1 do MESMO objeto nesta passagem.
                    # Caso contrário (objeto vindo no sentido oposto), não altera o contador.
                    if obj.get('counted', 0) == 1 and self.counter > 0:
                        self.counter -= 1
                        self._log(
                            f"RECONHECIMENTO {flow_mode.upper()} -1 {datetime.now().strftime('%d/%m/%Y %H:%M:%S')} (ID: {obj_id})"
                        )
                        ...
                        self._log(
                            f"IGNORADO {flow_mode.upper()} (passagem contrária sem +1 prévio) {datetime.now().strftime('%d/%m/%Y %H:%M:%S')} (ID: {obj_id})"
                        )
                        obj['counted'] = 0
                    obj['direction'] = 0
