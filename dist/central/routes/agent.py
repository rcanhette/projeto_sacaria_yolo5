...
def session_update():
    ...
    # Esperado: { agent_id, tc_id, session_db_id, total, increment, ts, log, event_id }
    tc_id = int(payload.get("tc_id")) if payload.get("tc_id") is not None else None
    ...
    try:
        if total is not None and delta is not None:
            insert_log(int(session_id), int(tc_id), int(delta), int(total), event_id=event_id)
    except Exception as e:
        logs.exception("[AGENT] fail to insert log: %s", e)
        return jsonify({"error": "db_error"}), 500

    # Atualiza espelho para SSE
    # UI deve zerar quando finalizar
    try:
        shadow.current_session_count = 0
    except Exception:
        pass
