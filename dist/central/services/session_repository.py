...
def insert_log(session_id: int, ct_id: int, delta: int, total_atual: int, event_id: str | None = None) -> None:
    if event_id:
        sql = """
            INSERT INTO session_log (session_id, ct_id, ts, delta, total_atual, event_id)
            VALUES (%s, %s, NOW(), %s, %s, %s)
            ON CONFLICT (event_id) DO NOTHING
        """
        execute(sql, [session_id, ct_id, delta, total_atual, event_id])
    else:
        sql = """
